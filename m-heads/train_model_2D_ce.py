...
def train_func_2d(mydict):
    ...
    for epoch in range(mydict["start_epoch"], mydict['num_epochs']):
        ...
        for step, (_,patch, mask) in enumerate(training_generator): 
            ...
            y_pred = network(patch) # using sigmoid as per xiaoling's implementation
            loss_val = 0.
            for mh in range(len(y_pred)):
                loss_val += torch.mean(train_loss_func(torch.sigmoid(y_pred[mh]), mask))
        ...
        with torch.no_grad():
            ...
            for _ in range(len(validation_generator)):
                ...
                y_pred = network(x)
                y_pred_avg = torch.sigmoid(y_pred[0])
                for mh in range(1,len(y_pred)):
                    y_pred_avg += torch.sigmoid(y_pred[mh])
                y_pred_avg /= len(y_pred)
                avg_val_loss += torch_dice_fn_ce(y_pred_avg, y_gt)
...
def test_func(mydict):
    ...
    for ind in range(num_samples):
        ...
        y_pred_list = network(torch_img) # NCDHW

        for mh in range(len(y_pred_list)):
            y_pred = torch.squeeze(torch.sigmoid(y_pred_list[mh])).detach().numpy()
            #y_pred[y_pred < 0.5] = 0.
            #y_pred[y_pred >= 0.5] = 255.
            y_pred = np.clip(y_pred * 255., 0., 255.)
            SaveFiles(mydict['output_folder'], filename+"_{}".format(mh), y_pred)
