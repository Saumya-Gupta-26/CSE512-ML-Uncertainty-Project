...
def train_func_2d(mydict):
    ...
    # Network
    network = ProbabilisticUnet(input_channels=1, num_classes=mydict['num_classes'], num_filters=[32,64,128,192], latent_dim=2, no_convs_fcomb=4, beta=10.0)
    ...
    for epoch in range(mydict["start_epoch"], mydict['num_epochs']):
        ...
        for step, (_,patch, mask) in enumerate(training_generator): 
            ...
            network.forward(patch,mask,training=True)
            elbo = network.elbo(mask)
            reg_loss = l2_regularisation(network.posterior) + l2_regularisation(network.prior) + l2_regularisation(network.fcomb.layers)
        ...
        with torch.no_grad():
            ...
            for _ in range(len(validation_generator)):
                ...
                network.forward(x,y_gt,training=False)
                y_pred = network.sample(testing=True)
...
def train_func_2d_cityscapes(mydict):
    ...
    # Network
    network = ProbabilisticUnet(input_channels=3, num_classes=mydict['num_classes'], num_filters=[32,64,128,192], latent_dim=2, no_convs_fcomb=4, beta=10.0)
...
def test_func(mydict):
    ...
    for ind in range(num_samples):
        filename, torch_img, torch_gt = test_set.__getitem__(ind)

        torch_img = torch.unsqueeze(torch_img, dim=0).cuda() # to include batch=1 dimension
        torch_gt = torch.unsqueeze(torch_gt, dim=0).cuda() # to include batch=1 dimension

        network.forward(torch_img,torch_gt,training=False)

        for idxx in range(num_itr):
            y_pred = network.sample(testing=True)
            y_pred = torch.squeeze(y_pred).cpu().detach().numpy()
            #y_pred[y_pred < 0.5] = 0.
            #y_pred[y_pred >= 0.5] = 255.
            y_pred = np.clip(y_pred * 255., 0., 255.)
            SaveFiles(mydict['output_folder'], filename+"_{}".format(idxx), y_pred)
