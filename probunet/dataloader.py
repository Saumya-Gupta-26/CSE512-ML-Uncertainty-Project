...
class Dataset2D_cityscapes(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index): # return CHW torch tensor
        ...
        for ch in range(np_img.shape[2]):
            meanval = np_img[:,:,ch].mean()
            stdval = np_img[:,:,ch].std()

            if stdval == 0.0:
                np_img[:,:,ch] = np_img[:,:,ch]/meanval
            else:
                np_img[:,:,ch] = (np_img[:,:,ch] - meanval) / stdval
        ...
        return imgpath.split('/')[-1].replace(img_suffix, ""), torch_img, torch_gt 





# uses filelist for input
